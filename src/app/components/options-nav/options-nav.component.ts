...
@Component({
  ...
  template: `
    ...
    <div class="block md:hidden">
      <div class="w-full">
        <button (click)="toggleMenu()" class="w-full bg-gray-100 dark:bg-dark dark:text-white py-2 border-b-4 border-b-gold">
          {{ division }}
        ...
        @if (isMenuOpen) {
          <div class="flex flex-col bg-gray-100 dark:bg-dark w-full border-b-4 border-b-gold">
            @for (item of options; track $index) {
              <a [routerLink]="item.route" class="text-center py-2 text-neutral-600 dark:text-neutral-200" (click)="toggleMenu()">
